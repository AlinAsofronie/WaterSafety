...
export async function GET() {
  try {
    ...
    const filterExpiryAssets = allAssets.filter(asset => {
      ...
      // Use the exact same logic as the web application dashboard - only ACTIVE assets (v2)
      ...
      if (asset.assetBarcode === 'B30674' || allAssets.indexOf(asset) < 5) {
        console.log(`Debug asset ${asset.assetBarcode}: expired=${isExpired}, expiringThisWeek=${isExpiringThisWeek}, filterNeeded=${filterNeeded}, filtersOn=${filtersOn}, isActive=${isActive}, status=${asset.status}, expiryDate=${asset.filterExpiryDate}, daysUntilExpiry=${daysUntilExpiry}`);
      }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        description: "ACTIVE assets with filters that are expired or expiring this week (Monday to Sunday) - Updated"
      },
