...
const LPManagement = dynamic(() => import('@/components/LPManagement'), {
  loading: () => (
    <Stack align="center" gap="md" mt="xl">
      <Loader size="lg" />
      <Text c="dimmed">Loading LP Management...</Text>
    </Stack>
  ),
  ssr: false
});

const AssetReconciliation = dynamic(() => import('@/components/AssetReconciliation'), {
  loading: () => (
    <Stack align="center" gap="md" mt="xl">
      <Loader size="lg" />
      <Text c="dimmed">Loading Asset Reconciliation...</Text>
    </Stack>
  ),
  ssr: false
});

const EmailReportsManager = dynamic(() => import('@/components/EmailReportsManager'), {
  loading: () => (
    <Stack align="center" gap="md" mt="xl">
      <Loader size="lg" />
      <Text c="dimmed">Loading Email Reports Manager...</Text>
    </Stack>
  ),
  ssr: false
});
...
export default function HomePage() {
  // Lazy load XLSX library only when needed (saves ~500KB initial bundle)
  ...
  const downloadBulkUpdateTemplate = async (format: 'csv' | 'excel' = 'csv', includeData: boolean = false) => {
    try {
      console.log(`Downloading bulk update template: format=${format}, includeData=${includeData}`);
      ...
      if (response.ok) {
        ...
        if (contentType && contentType.includes('application/json')) {
          // This is an error response disguised as JSON
        ...
        // Try to read a small portion of the blob to verify it's not an error
        const text = await blob.slice(0, 100).text();
        console.log('Blob content preview:', text);
        
        // Check if the blob content looks like an error response
        if (text.includes('"success":false') || text.includes('"error"')) {
          console.error('Blob contains error response:', text);
          throw new Error('Server returned error in file response');
        }

        // Create download link with enhanced compatibility
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        const dateTimeStr = getDateTimeString();
        const suffix = includeData ? 'with-data' : 'template';
        const filename = `bulk-update-${suffix}_${dateTimeStr}.${format === 'excel' ? 'xlsx' : 'csv'}`;
        a.download = filename;
        ...
        // Enhanced download trigger with multiple fallbacks
        try {
          a.click();
          console.log('Download triggered via click()');
        } catch (clickError) {
          console.error('Click method failed, trying alternatives:', clickError);
          
          // Fallback 1: Programmatic click event
          try {
            ...
            console.error('Event dispatch failed, trying window.open:', eventError);
            
            // Fallback 2: Open in new window/tab
            try {
              window.open(url, '_blank');
              console.log('Download triggered via window.open()');
            } catch (openError) {
              console.error('All download methods failed:', openError);
              throw new Error('Browser blocked download - please check popup blockers');
            }
          }
        }
      ...
      notifications.show({
        title: 'Download Failed',
        message: `Failed to download bulk update template: ${errorMessage}`,
        color: 'red',
        icon: <IconX size={16} />,
        autoClose: 10000,
      });
  ...
  const renderReports = () => {
    ...
    const downloadFilterExpiryList = async () => {
      // Lazy load XLSX library only when needed
      const XLSX = await loadXLSX();
    ...
    const downloadFilterRemovalList = async () => {
      // Lazy load XLSX library only when needed
      const XLSX = await loadXLSX();
