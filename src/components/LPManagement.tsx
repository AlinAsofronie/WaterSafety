...
export default function LPManagement({ assets, onAssetClick }: LPManagementProps) {
  ...
  const formatDate = (dateString: string | Date | null | undefined): string => {
    ...
    // Try to parse as date and convert to YYYY-MM-DD format using UTC
    try {
  ...
  // FIXED: Properly handle Excel date serial numbers and timezone issues
  const convertDateFormat = (dateString: string | number | null | undefined): string => {
    if (!dateString) return '';
    
    // Handle Excel date numbers first - fix timezone issue
    if (typeof dateString === 'number') {
      // Excel uses serial numbers where 1 = January 1, 1900
      // Standard conversion: (Excel serial - 25569) * 86400 * 1000
      // Where 25569 = days between Jan 1, 1900 and Jan 1, 1970 (Unix epoch)
      // Use UTC to avoid timezone shifts
      const excelEpoch = Date.UTC(1899, 11, 30); // December 30, 1899, 00:00:00 UTC
      const date = new Date(excelEpoch + (dateString - 1) * 86400 * 1000);
    ...
    if (dateStr.match(/^\d{4}-\d{2}-\d{2}$/)) {
      return dateStr; // Already in correct format, return as-is
    }
    
    // Convert dd/mm/yyyy to ISO format - use UTC to avoid timezone shifts
    ...
    // Try to parse as a date and convert to ISO (YYYY-MM-DD format only, no time)
    try {
